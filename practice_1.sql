...
SELECT extract(YEAR FROM hire_date) as year, count(*) as hired_employee FROM employees
GROUP BY year
order by year DESC
...
SELECT customer_id, count(*) as order, sum(total_amount) as amount FROM orders
